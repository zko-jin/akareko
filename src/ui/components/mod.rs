...
use freya::{
    prelude::*,
    query::{self, Mutation, Query, UseMutation, UseQuery, use_mutation, use_query},
};
...
use crate::{
    ...
    ui::{
        Route, RouteContext,
        icons::{DOWNLOAD_ICON, SEEN_ICON, UNSEEN_ICON},
        queries::{AddTorrent, FetchContents, FetchTorrentStatus, UpdateContentProgress},
    },
};
...
impl<I: IndexTag + VisualizeRoute<I>> Component for ContentEntry<I> {
    fn render(&self) -> impl IntoElement {
        ...
        let torrent_status = use_query(
            Query::new(info_hash, FetchTorrentStatus).interval_time(Duration::from_secs(5)),
        );
        ...
        let watch_icon = {
            ...
            if content.progress >= content.count {
                watch_icon = watch_icon.child(svg(SEEN_ICON)).on_press(move |_| {
                    seen_mutation.mutate((content.signature().clone(), content.count));
                });
            } else {
                watch_icon = watch_icon.child(svg(UNSEEN_ICON)).on_press(move |_| {
        ...
        let (torrent_status_icon, on_press_title): (
            ...
            query::QueryStateData::Pending | query::QueryStateData::Loading { .. } => {
                (CircularLoader::new().into_element(), None)
            }
            query::QueryStateData::Settled {
                res: Ok(status), ..
            } => {
                let content = self.content.clone();
                ...
                match status {
                    Some(s) => match &s.state {
                        ...
                        anawt::TorrentState::Downloading => {
                            (ProgressBar::new(s.progress as f32).into_element(), None)
                        }
                    ...
                    None => {
                        ...
                        println!("Good to mutate");
                        let download_torrent: EventHandler<Event<PressEventData>> = (move |_| {
                            println!("MUTATING SHIT");
                            download_mutation.mutate(keys.clone());
                        })
                        .into();
                        (
                            Button::new()
                                .child(svg(DOWNLOAD_ICON).on_press(download_torrent.clone()))
                                .into_element(),
            ...
            query::QueryStateData::Settled { res: Err(e), .. } => (
